...
export default function Home() {
  ...
  const [rightTab, setRightTab] = useState<'telemetry' | 'translate' | 'vocabulary' | 'practice'>('telemetry');
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-[#06090F] text-slate-100 font-sans selection:bg-brand-emerald selection:text-black">
      ...
      {/* Master Responsive Hackathon Presentation Dashboard */}
      <main className="flex-1 max-w-[1600px] w-full mx-auto p-3 sm:p-5 lg:p-6 space-y-4">
        {/* If user navigated via top nav, keep active view or master dual-column view */}
        {activeTab === 'vision' ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            {/* Right Column (5 cols): High-Impact Tabbed Sidebar */}
            <div className="lg:col-span-5 flex flex-col space-y-3">
              {/* Right Sidebar Quick Switcher Tabs */}
              <div className="flex items-center bg-surface-100 p-1.5 rounded-xl border border-surface-200 shadow-inner gap-1">
                <button
      ...
      <footer className="border-t border-surface-200 bg-surface-50/60 py-4 px-4 sm:px-8 mt-6">
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row items-center justify-between gap-3 text-xs text-slate-400">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-white">SignBridge:</span>
            <span className="font-mono text-slate-400 hidden lg:inline">
              Webcam (30 FPS) &rarr; MediaPipe (21/33 Pts) &rarr; Scale-Invariant Kinematics (63D) &rarr; Rolling Buffer (30 Frames) &rarr; Dual Classifier &rarr; Offline TTS
            </span>
          </div>

          <div className="flex items-center gap-3 text-slate-400 font-mono text-[11px]">
            <span className="text-brand-emerald font-semibold">🔒 100% Client-Side Privacy</span>
            <span>•</span>
            <span className="text-brand-cyan">Zero Cloud API Cost</span>
          </div>
        </div>
      </footer>
    </div>
